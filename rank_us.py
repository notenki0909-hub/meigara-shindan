...
def _fmt_zone_num(v, unit):
    """しきい値の表示用フォーマット。JP版rank.pyの_fmt_zone_numと同じ流儀。"""
    if unit == "倍":
        return f"{v:.2f}{unit}"
  ...
  <div class="fmaj"><h3 class="fmajh">① 銘柄選定</h3>
    ...
    <div class="fsec"><span class="fsech">配当の持続力</span><div class="fsecbody">
      <div class="fgrp"><label class="flbl" for="f_dg">増配率(%) 以上</label><input id="f_dg" type="number" step="0.1">
        <span class="fchecks">{"".join(f'<label><input type="checkbox" class="f_dg" value="{v}">{lab}</label>' for v, lab in zip(("cheap","normal","expensive"), _zone_labels("higher_better",5,0,"%",("良好","注意","弱い"))))}</span></div>
      <div class="fgrp"><label class="flbl" for="f_strup">連続増配年数 以上</label><input id="f_strup" type="number" min="0">
        <span class="fchecks">{"".join(f'<label><input type="checkbox" class="f_strup" value="{v}">{lab}</label>' for v, lab in zip(("cheap","normal","expensive"), _zone_labels("higher_better",15,5,"年",("良好","注意","弱い"))))}</span></div>
      <div class="fgrp"><label class="flbl" for="f_strflat">連続非減配年数 以上</label><input id="f_strflat" type="number" min="0">
        <span class="fchecks">{"".join(f'<label><input type="checkbox" class="f_strflat" value="{v}">{lab}</label>' for v, lab in zip(("cheap","normal","expensive"), _zone_labels("higher_better",20,5,"年",("良好","注意","弱い"))))}</span></div>
  ...
  <div class="fmaj"><h3 class="fmajh">② 買い時</h3>
    ...
    <div class="fsec"><span class="fsech">利回り水準とChowder</span><div class="fsecbody">
      <div class="fgrp"><label class="flbl" for="f_yld">予想配当利回り(%) 以上</label><input id="f_yld" type="number" step="0.1" min="0">
        <span class="fchecks">{"".join(f'<label><input type="checkbox" class="f_yld" value="{v}">{lab}</label>' for v, lab in zip(("cheap","normal","expensive"), _zone_labels("higher_better",3.0,1.5,"%",("良好","注意","弱い"))))}</span></div>
...
(function(){{
  ...
  // 割安/標準/割高＝sector_rules_us.jsonの採点しきい値（good/warn）、良好/注意/弱い＝bar()のグループ
  // スコア閾値（80/62）と同じ境界で、それぞれ◎/△/▲を3区分に分ける（内部値はcheap/normal/expensiveで共通化）
  var BAND_FILTERS = [
    ...
    ['dg','dgr5','higher_better',5,0],
    ['strup','streakup','higher_better',15,5],
  ...
  document.getElementById('csvExport').addEventListener('click', function(){{
    ...
    a.href = url;
    a.download = 'meigara_shindan_us_' + new Date().toISOString().slice(0,10) + '.csv';
