...
def build_metrics(yd, irbank, sec_avg, is_simple, jp_sector, rate_sensitive, jgb_10y, div_policies):
    ...
    M["業績"].append({"name": "利益の安定度（営業利益のブレ）", "v": earn_stab, "disp": es_disp,
                      "ref": "1に近いほどブレが小さい（0.88以上で安定・0.65未満は景気敏感）", "key": "earnings_stability"})
...
METRIC_HELP = {
    ...
    "earnings_stability": {"what": "直近数期の営業利益で、いちばん落ち込んだ年の「前年比」。1に近いほど利益がブレず、配当を出し続けやすい。景気敏感株（商社・海運・鉄鋼）は低く、内需ディフェンシブ（通信・食品）は高い。増配が伸びより『途切れないこと』を配当投資家は重視する。", "unit": ""},
